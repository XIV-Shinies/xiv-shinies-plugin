...
| **Explicit opt-in before non-essential data collection; no silent first-run behavior** | Consent is enforced in code, not just reflected in UI: `UploadGate.CanContactServer` requires completed onboarding **and** the master switch **and** a usable token before any request — including the config poll. A fresh install talks to nobody. The first-run wizard discloses exactly what each category sends before the user can enable it. The live occult tracker's toggle is the one consent that defaults ON — it shares world state, and the default is defensible only because the ticked box is **visible on the wizard's consent step** before anything can send; a config from before the toggle existed is migrated to OFF at load, because that user's wizard ran long ago and a silent default would be consent by omission. The same migration turns off `AutoEnableNewFeatures`, the user's standing choice about future sharing features, for the same reason. | `Sync/UploadGate.cs`, `Occult/OccultGate.cs`, `PluginSettings.ApplyUpgradeMigrations`, `Windows/MainWindow.Wizard.cs` |
